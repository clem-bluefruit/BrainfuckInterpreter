...
TEST_F(Brainfuck, Interpreter_adds_9_to_first_and_second_cells_Output_is_TABTAB)
{
	reader.UserCodeInput("+++++++++>+++++++++");
...
TEST_F(Brainfuck, Initialised_tapecell_rollover_behavior_first_cell_subtract_1_Output_is_CHAR255)
{
	reader.UserCodeInput("-");
	ASSERT_EQ("\xFF", reader.Parse()) << "Output is NOT '\xFF'";
}

TEST_F(Brainfuck, Skip_to_end_of_tape_then_skip_forward_to_second_cell_add_9_Output_is_TAB)
{
	reader.UserCodeInput("<>>+++++++++");
	ASSERT_EQ("\t", reader.Parse()) << "Tape skipping failed: Cell 2 is NOT TAB";
}

TEST_F(Brainfuck, Rollover_to_last_cell_subtract_1_Output_is_CHAR255)
{
	reader.UserCodeInput("<-");
	ASSERT_EQ("\xFF", reader.Parse()) << "Output is NOT '\xFF'";
}
